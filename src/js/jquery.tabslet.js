  ...
  $.fn.tabslet = function (options) {
    ...
    return this.each(function () {
      ...
      if (!$this.data('tabslet-init')) {
        ...
        _tabs.removeClass('tabactive');
        /* _tabs.addClass('tabdisabled'); */

        if ($this.opts.active) {
          _tabs.eq($this.opts.active - 1).addClass('tabactive');
          elements.eq($this.opts.active - 1).addClass(options.activeclass);
        }

        var fn = eval(function (e, tab) {
          var _this = tab
            ? elements.find('a[' + $this.opts.attribute + '="' + tab + '"]').parent()
            : $(this);
          ...
          if ($this.opts.animation) {
            ...
            _container.find(currentTab).addClass('tabactive');
            _container.find(currentTab).removeClass('tabdisabled');
            _this.trigger('_after');
          }
